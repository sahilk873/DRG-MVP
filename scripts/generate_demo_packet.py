...
AUTOMATION_OUTPUT = ROOT / "demo/src/fixtures/automation-plan.json"
...
def main() -> int:
    ...
    expected, expected_automation = build_fixtures()
    if args.check:
        stale = [
            path for path, content in (
                (OUTPUT, expected), (AUTOMATION_OUTPUT, expected_automation)
            )
    ...
    AUTOMATION_OUTPUT.write_text(expected_automation, encoding="utf-8")
    print(OUTPUT.relative_to(ROOT), AUTOMATION_OUTPUT.relative_to(ROOT))
    return 0
